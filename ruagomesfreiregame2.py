...
while (True):
    ...
    val = list([ii / NREP for ii in val])
    #print(val)
    cor = [(val[0]) >= 0.3, (val[1]) >= 0.3, (val[2]) >= -0.85,
           (val[3]) >= -0.6]
    # these values are not the optimal, they include some slack
    #print(cor)
    grade = 0
    ...
    mediaValor3 += val[2]
    if grade != 20:
        mediaValor3 = mediaValor3 / num20
        print(
            "Grade in these tests (the final will also include hidden tests) : ",
            grade)
        print('TIVESTE ', num20, 'TESTES COM 20')
        print('MEDIA DO VALOR 3:', mediaValor3)
        mediaValor3 = 0
        num20 = 0